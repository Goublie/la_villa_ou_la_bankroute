...
public class BoursePortefeuilleUI : MonoBehaviour
{
    [Header("Elements UI")]
    [SerializeField] private TableauScrollSelectable tableauActifs;
    [SerializeField] private RectTransform graphiqueRoot;
    [SerializeField] private RectTransform graphiqueCamembertRoot;
    [SerializeField] private TMP_Text recapActifText;
    [SerializeField] private TMP_InputField montantAchatInput;
    [SerializeField] private TMP_InputField montantVenteInput;
    [SerializeField] private TMP_Text cashText;
    [SerializeField] private TMP_Text valeurTotaleText;
    ...
    private GraphiqueBourseUI graphiqueCourbe;
    private GraphiqueCamembertBourse graphiqueCamembert;
    ...
    public void Vendre()
    {
        if (!EssayerLireMontantOrdre(montantVenteInput, out int montantCentimes, out string erreur))
        {
        ...
        {
            AfficherResultat(ResultatOperation.Echec("Veuillez sélectionner un actif à vendre.", "actif_non_selectionne"));
            return;
        }

        AfficherResultat(serviceBourse.VendreMontant(
            actifSelectionne,
            montantCentimes,
            MoisActuel,
            compteCourant,
    ...
    public void Acheter()
    {
        if (!EssayerLireMontantOrdre(montantAchatInput, out int montantCentimes, out string erreur))
        {
            AfficherResultat(ResultatOperation.Echec(erreur, "saisie_invalide"));
            return;
        }
    ...
    private void ActualiserAffichage()
    {
        ...
        if (valeurTotaleText != null && donneesBourse != null && serviceBourse != null)
        {
            long totalCentimes = 0;
            var actifs = CatalogueActifs.ObtenirActifs();
            foreach (var actif in actifs)
            {
                PositionBourse position = donneesBourse.TrouverPosition(actif.Id);
                if (position != null && position.quantite > 0)
                {
                    totalCentimes += serviceBourse.CalculerValeurPositionCentimes(position, MoisActuel);
                }
            }
            valeurTotaleText.text = "Valeur totale : " + FormaterArgent((int)totalCentimes);
        }
        ...
        if (actifSelectionne != null)
        {
            ...
            ActualiserGraphiqueCourbe(actifSelectionne);
        }
        else
        {
            if (recapActifText != null)
    ...
    private void RemplirTableau()
    {
        ...
        foreach (var actif in actifs)
        {
            PositionBourse position = donneesBourse != null ? donneesBourse.TrouverPosition(actif.Id) : null;
            float qte = position != null ? position.quantite : 0f;

            if (qte <= 0f)
            {
                continue; // N'afficher que les actions détenues
            }

            // Colonnes: Nom | Prix unitaire | Gain relatif | Gain brut | Nombre de parts
    ...
    private void RafraichirPrixTableau()
    {
        ...
        foreach (var kvp in mapActifs)
        {
            ...
            // On met à jour la case 1 (Prix), la case 2 (Variation relative), la case 3 (Variation absolue), et la case 4 (Quantité)
    ...
    private void ActualiserGraphiqueCourbe(DefinitionActifFinancier actif)
    {
        if (graphiqueRoot == null) return;
        
        graphiqueCourbe = graphiqueCourbe ?? new GraphiqueBourseUI(graphiqueRoot);
        graphiqueCourbe.Afficher(actif, serviceBourse, MoisActuel);
    }

    private void ActualiserGraphiqueCamembert()
    {
        if (graphiqueCamembertRoot == null) return;
        
        graphiqueCamembert = graphiqueCamembert ?? new GraphiqueCamembertBourse(graphiqueCamembertRoot);
        graphiqueCamembert.Afficher(donneesBourse);
    }
    ...
    {
        if (montantAchatInput != null)
        {
            montantAchatInput.contentType = TMP_InputField.ContentType.IntegerNumber;
            montantAchatInput.characterValidation = TMP_InputField.CharacterValidation.Integer;
            montantAchatInput.characterLimit = 8;
            if (string.IsNullOrWhiteSpace(montantAchatInput.text))
            {
                montantAchatInput.SetTextWithoutNotify("500");
            }
        }

        if (montantVenteInput != null)
        {
            montantVenteInput.contentType = TMP_InputField.ContentType.IntegerNumber;
            montantVenteInput.characterValidation = TMP_InputField.CharacterValidation.Integer;
            montantVenteInput.characterLimit = 8;
            if (string.IsNullOrWhiteSpace(montantVenteInput.text))
            {
                montantVenteInput.SetTextWithoutNotify("500");
            }
        }
    }

    private bool EssayerLireMontantOrdre(TMP_InputField inputField, out int montantCentimes, out string erreur)
    {
        montantCentimes = 0;
        erreur = string.Empty;
        ...
        string texte = inputField.text != null ? inputField.text.Trim() : string.Empty;
